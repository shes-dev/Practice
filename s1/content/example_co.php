...
// Connecting End
	//if ($Li_Number_All == 'a')
	//{
	$sql1='SELECT * FROM table_co_'.$Li_Number.' ORDER BY Date DESC';
	//}
	/*if ($Li_Number_All == 'n')
	{
	$sql1='SELECT * FROM table_co WHERE Belongsto = '.$Li_Number.' ORDER BY Date DESC';
	}*/
	
	/*
	echo '<pre style="background:white;color:black;margin-top:3px;text-align:center;border:2px black solid;">';
	echo '<h3 style="margin:0px auto;"><u>co\sql:</u></h3>';
...
{	/*
	...
	*/	
	if ($row['Type']=='co')
		{
			echo '<b><li class="comment_'.$i.'" id="'.$row['Type'].'_'.$row['Belongsto'].'">';
			echo $row['Content'].'</b><br>';
			echo $row['Date'];
			echo '</li>';
			//echo '<i id="plus_'.$i.'" class="fa fa-plus-square-o square-plus"></i> <i id="check_circle_'.$i.'" class="fa fa-check-circle check-circle"></i> <i id="times_circle_'.$i.'" class="fa fa-times-circle times-circle"></i> <i id="toggle_down_'.$i.'" class="fa fa-toggle-down"></i>';
			echo '<i id="comment_check_circle_'.$row['Belongsto'].'" class="fa fa-check-circle check-circle_co" style="font-size:24px;"></i> <i id="comment_times_circle_'.$row['Belongsto'].'" class="fa fa-times-circle times-circle_co" style="font-size:24px;"></i>';
			$i++;
		}
}
